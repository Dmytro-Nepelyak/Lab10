...
public class MySQLDekanat {
    private static final String DB_URL = "jdbc:mysql://localhost:3306/dekanat";
    private static final String DB_USER = "root";
    private static final String DB_PASS = "123111";
    ...
    public static void createTables() {
        try (Connection conn = connect(); Statement stmt = conn.createStatement()) {
            ...
            System.out.println("Таблиці створені");
        ...
        try (Connection conn = connect(); Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
            while (rs.next()) {
                System.out.printf("ID: %d | Ім'я: %s | Середній бал: %.2f | Дата народження: %s%n",
                        ...
                        rs.getDouble("avg_grade"), rs.getDate("birth_date"));
            }
        } catch (SQLException e) {
